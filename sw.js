...
const CACHE_NAME = 'flight-assets-v4';
...
// Install: precache app shell + images
...
// overwrite the cached copy, then tell all clients to reload. This is the reliable
// way to push a new version without relying on iOS's lazy cache behavior.
self.addEventListener('message', event => {
  if (event.data && event.data.type === 'FORCE_UPDATE') {
    event.waitUntil((async () => {
      try {
        const cache = await caches.open(CACHE_NAME);
        // Fetch fresh, cache-busting at the network layer
...
// Fetch:
//  - Navigation requests (loading the app page) and known shell assets: cache-first,
//    with a fast (timeout-bounded) network attempt in the background to refresh the
//    cache for next time. This is what makes "online but can't actually reach the
//    site" behave like a normal offline load instead of a failure.
//  - Known image assets: cache-first, network fallback (unchanged behavior).
  ...
  if (isNavigation || isShellAsset) {
    event.respondWith(
      caches.open(CACHE_NAME).then(async cache => {
        ...
        const networkUpdate = fetchWithTimeout(req, NETWORK_TIMEOUT_MS)
          .then(async response => {
            if (response && response.ok) {
              // Compare against cached version — notify clients if content changed
        ...
        if (cached) {
          // Serve cached shell immediately; let the network update happen silently
          networkUpdate.catch(() => {});
          return cached;
        }

        // Nothing cached yet — wait for the (fast-timeout) network attempt
        const fresh = await networkUpdate;
        if (fresh) return fresh;

        // No cache, no network — surface a minimal offline notice rather than a hard failure
        return new Response(
